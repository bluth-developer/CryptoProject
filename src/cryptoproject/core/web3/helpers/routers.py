...
from core.misc.exceptions import NotSupportedDEXError, NotSupportedChainDEXError

abi_router_v2 = [{"inputs": [{"internalType": "address", "name": "_factory", "type": "address"},
...
exchanges = {
    "sushiswap_v2": {"chains": {"arbitrum_one": "0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506"}, "abi": abi_router_v2}
}


def load_router(exchange: str, chain: str):
    """
    :param chain: - Blockchain name
    :param exchange: - DEX name
    :return:
    Router Contract ( address, abi ) of "exchange" if exists,
    else raises exceptions.NotSupportedDEX
    """
    loggers.dex.debug(f"Loading DEX \"{exchange}\" router in {chain}")
    router = exchanges.get(exchange)
    if not router:
        raise NotSupportedDEXError(exchange)
    address = router['chains'].get(chain)
    if not address:
        raise NotSupportedChainDEXError(exchange, chain)
    return {'abi': router['abi'], 'address': address}
